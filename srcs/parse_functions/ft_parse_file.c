...
int		ft_is_config_valid(t_config *config)
{
	...
	(!config->path_east) ? error_value = PATH_EAST_ERROR : 0;
	(!config->path_west) ? error_value = PATH_WEST_ERROR : 0;
	(!config->path_north) ? error_value = PATH_NORTH_ERROR : 0;
	(!config->path_south) ? error_value = PATH_SOUTH_ERROR : 0;
	(!config->path_sprite) ? error_value = PATH_SPRITE_ERROR : 0;
...
int		ft_parse_file(t_config *config, char *cub_file)
{
	...
	while (get_next_line(fd, &line) == 1)
	{
		...
		(line[0] == '1') ? config->map = ft_get_map(config, &line, fd) : 0;
	}
