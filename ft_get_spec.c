...
/*   Updated: 2019/11/27 19:13:21 by abarot           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_get_spec(t_spec *spec, int fd)
{
	...
	while ((res =ft_get_next_line(fd, line)) == 1)
	{
		...
		(line[0][0] == 'S' && line[0][1] == ' ') ? spec->path_sprite = ft_strdup(*(line + 2)) : 0;
	...
	free(line)
	if (!ft_spec_isvalid(spec))
		return (0);
	return (1);
}

void ft_get_map(int **map, char *line, int fd)
{
...
void	ft_get_col(t_col *col, char *line, int case)
(
	int i_line;
...
int	ft_spec_isvalid(t_spec *spec)
{
	...
	&& (spec->col_ceil->B == spec->col_floor->B))
		return (0);
	if ( !spec->path_north || !spec->path_west = 0 || !spec->path_east || !spec->path_south || !spec->path_sprit)
		return (0);
	if (!ft_check_map(spec->map))
		return (0)
	return (1);
}
